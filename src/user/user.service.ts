...
export class UserService {
    ...
    async registerUser(dto: CreateUserRequestDto): Promise<BaseResponse> {
        // console.log(dto)
        if (!dto.email || !dto.password || !dto.username) throw new BadRequestException('Missing required fields');
    ...
    async login({email, password}: {email: string; password: string}): Promise<BaseResponse> {
        if (!email || !password) throw new BadRequestException('Missing required fields ');

        const user: User | null = await this.userModel.findOne({email});
        if (!user) throw new NotFoundException('No user found with this email');
        const verify_password: boolean = await compare(password, user?.password);
        if (!verify_password) throw new UnauthorizedException('Invalid Password');
        const payload = {
            email: user.email,
            user_id: user.id,
        };
    ...
    async auth0Login(data: {email: string; username: string; avatar?: string}): Promise<BaseResponse> {
